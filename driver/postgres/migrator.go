...
// Migrator implements the dialect.Migrator interface for PostgreSQL.
type Migrator struct {
	driver *Driver
}

func (m *Migrator) AutoMigrate(models ...interface{}) error {
	inner := migrate.NewMigrator(&pgDB{m.driver.db}, m.driver)
...
func (m *Migrator) CreateTable(name string, models ...interface{}) error {
	...
	_, err := m.driver.db.Exec("CREATE TABLE IF NOT EXISTS " + quote(name) + " (id BIGSERIAL PRIMARY KEY)")
	return err
}
...
func (m *Migrator) AlterColumn(table, column, newType string) error {
	_, err := m.driver.db.Exec(fmt.Sprintf("ALTER TABLE %s ALTER COLUMN %s TYPE %s", quote(table), quote(column), newType))
	return err
}
...
// quote wraps a PostgreSQL identifier in double quotes.
func quote(name string) string {
	return `"` + strings.ReplaceAll(name, `"`, "") + `"`
}

// pgDB wraps *sql.DB to satisfy migrate.DB.
type pgDB struct{ db *sql.DB }

func (s *pgDB) Exec(ctx context.Context, q string, args ...interface{}) error {
	_, err := s.db.ExecContext(ctx, q, args...)
	return err
}

func (s *pgDB) Query(ctx context.Context, q string, args ...interface{}) (migrate.Result, error) {
